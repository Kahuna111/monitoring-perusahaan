...
$pageTitle  = 'Catat Uang Keluar';
$activePage = 'pengeluaran';
$breadcrumb = 'Uang Keluar / Tambah Baru';

$errors = [];

// Ambil kategori pengeluaran
$kategoriList = $pdo->query("SELECT * FROM kategori_transaksi WHERE tipe = 'pengeluaran' ORDER BY nama")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($jumlah <= 0)       $errors[] = 'Jumlah pengeluaran harus lebih besar dari 0.';
    ...
    if (isset($_FILES['bukti']) && $_FILES['bukti']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            ...
            if ($fileSize <= 2 * 1024 * 1024) {
                $newFileName = 'keluar_' . time() . '_' . md5(uniqid()) . '.' . $fileExtension;
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO pengeluaran (tanggal, kategori_id, deskripsi, jumlah, bukti, dibuat_oleh) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$tanggal, $kategoriId, $deskripsi, $jumlah, $buktiName, $dibuatOleh]);

            setFlash('success', 'Transaksi uang keluar berhasil dicatat.');
...
<div class="page-header">
    <div class="page-header-left">
        <h2>Catat Uang Keluar</h2>
        <p>Input transaksi pengeluaran kas perusahaan secara berkala</p>
    </div>
    <a href="<?= BASE_URL ?>/admin/pengeluaran/index.php" class="btn btn-outline">
...
<form method="POST" enctype="multipart/form-data" data-validate>
    ...
    <div class="grid-2">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label class="form-label">Jumlah Uang Keluar <span class="required">*</span></label>
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <textarea name="deskripsi" class="form-control" rows="4" placeholder="Masukkan detail informasi transaksi pengeluaran ini..."><?= htmlspecialchars($_POST['deskripsi'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <!-- Petunjuk -->
            <div class="card" style="border:1.5px solid #fee2e2;background:#fef2f2;">
                <div class="card-body">
                    <div style="font-size:13.5px;font-weight:700;color:#991b1b;margin-bottom:8px;display:flex;align-items:center;gap:8px;">
                    ...
                    <ul style="font-size:13px;color:#991b1b;line-height:1.8;padding-left:16px;">
                        <li>Pencatatan pengeluaran harus disertai dengan nota atau kwitansi yang sah.</li>
                        <li>Pastikan jumlah uang keluar sudah dinegosiasikan dengan baik sebelum dicatat.</li>
                        <li>Disarankan mengunggah bukti transaksi transfer bank atau kwitansi cetak.</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:20px;">
        <a href="<?= BASE_URL ?>/admin/pengeluaran/index.php" class="btn btn-outline">Batal</a>
